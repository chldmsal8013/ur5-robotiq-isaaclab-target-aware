...
class PPO:
    ...
    def update(self):  # noqa: C901
        ...
        for (
            ...
            # [PATCH v2] Skip optimizer step if grad_norm is NaN/Inf to avoid corrupting parameters
            if torch.isfinite(grad_norm):
                self.optimizer.step()
            else:
                print(f"[WARNING] Skipping optimizer step: grad_norm={grad_norm.item()}")
                self.optimizer.zero_grad()
            # [PATCH v2] Sanitize log_std: nan_to_num BEFORE clamp (clamp does not fix NaN)
            with torch.no_grad():
                if hasattr(self.policy, 'log_std') and self.policy.log_std is not None:
                    self.policy.log_std.data = torch.nan_to_num(
                        self.policy.log_std.data, nan=0.0, posinf=0.5, neginf=-4.0
                    )
                    self.policy.log_std.data.clamp_(min=-4.0, max=0.5)
            # -- For RND
